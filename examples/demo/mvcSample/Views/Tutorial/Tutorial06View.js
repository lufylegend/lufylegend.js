function Tutorial06View(){
	base(this,LView,[]);
}
Tutorial06View.prototype.construct=function(){
};
Tutorial06View.prototype.init=function(){
	var self = this;
...
Tutorial06View.prototype.talkChangeEvent=function(){
	var self = this;
	self.addEventListener(LMouseEvent.MOUSE_UP,self.talkChange);
};
Tutorial06View.prototype.talkChange=function(event){
	var self = event.clickTarget;
	self.removeEventListener(LMouseEvent.MOUSE_UP,self.talkChange);
	self.controller.talkChange();
};
Tutorial06View.prototype.setTalk=function(data){
	var self = this;
	Talk(self,data.index,data.sub_index,data.message,function(){
		self.talkChangeEvent();
	});
};
Tutorial06View.prototype.handShow=function(){
	var self = this;
	self.addHand(120,700);
	Talk(0,1,"强盗已经到了城外，请点击［作战］按钮，开始战斗吧。",function(){
		//self.addEvent();
	});
};

Tutorial06View.prototype.addHand=function(x,y){
	var self = this;
	var hand = new Hand();
	hand.x = x;
	hand.y = y;
...
Tutorial06View.prototype.menuInit=function(){
	var self = this;
	...
	buttonMap.addEventListener(LMouseEvent.MOUSE_UP,function(){
		//if(self.controller.tutorialing && self.controller.tutorialingGet)return;
		self.controller.toBigmap();
	});
	...
	/*buttonMain.addEventListener(LMouseEvent.MOUSE_UP,function(){console.log((self.controller.tutorialing +"&&"+ self.controller.tutorialingLvUp));
		if(self.controller.tutorialing && self.controller.tutorialingGet)return;
 ...
 * 背景
 */
Tutorial06View.prototype.backgroundInit=function(){
	var self = this;
...
Tutorial06View.prototype.buildInit = function(){
...
Tutorial06View.prototype.setBuild=function(path,x,y,name,lv){
	var self = this;
...
Tutorial06View.prototype.addEvent=function(){
	var self = this;
...
Tutorial06View.prototype.onDown=function(event){
	var self = event.clickTarget;
	self.downPoint = {x:event.offsetX,y:event.offsetY,bx:self.backLayer.x,by:self.backLayer.y};
};
Tutorial06View.prototype.onMove=function(event){
	var self = event.clickTarget;
...
Tutorial06View.prototype.onUp=function(event){
	var self = event.clickTarget;
...
Tutorial06View.prototype.cloudsInit=function(){
	var self = this;
...
Tutorial06View.prototype.showBuild = function(buildType){
	var self = this;
	self.die();
 ...
 * 上の顔画像や、ステータス部分
 */
Tutorial06View.prototype.statusInit=function(){
	var self = this;
