...
var LComboBox = (function () {
  function LComboBox(size, color, font, layerBack, layerUp, layerOver) {
    ...
    if (params.listViewPanel) {
      s.listView.backgroundPanel = params.listViewPanel;
      s.listView.addChildAt(params.listViewPanel, 0);
    }
    var layer;
    ...
    label.x = 4 + offsetX;
    label.y = (layerBack.getHeight() - s.size) / 2 + offsetY;
    label.text = "";
    label.size = s.size;
    label.color = s.color;
    label.font = s.font;
    ...
    s.label = label;
    s.layer = layer;
    s.listChildView = params.childView || LComboBoxChild;
  ...
  */
  ...
  LComboBox.prototype.showChildList = function (list, index) {
    ...
    var w = s.listView.cellWidth;
    var h = s.listView.cellHeight * (s.list.length > s.maxIndex ? s.maxIndex : s.list.length);
    if (s.listView.backgroundPanel) {
      s.listView.backgroundPanel.resize(w, h);
    }
    s.listView.resize(w, h);
