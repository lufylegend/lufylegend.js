...
export class LListView extends LSprite {
    ...
    clickOnChild(selfX, selfY, type) {
        ...
        if (index < self._ll_items.length) {
            ...
            let event = { currentTarget: self, target: child, offsetX: LGlobal.offsetX, offsetY: LGlobal.offsetY, selfX: (x % self.cellWidth), selfY: (y % self.cellHeight) };
            if (type === 'touch') {
                child.onTouch(event);
            } else {
                child.onClick(event);
...
class LListViewDragObject extends LSprite {
    constructor(listView, selfX, selfY) {
        ...
        self.sx = self.x = LGlobal.offsetX;
        self.sy = self.y = LGlobal.offsetY;
    ...
    _ll_focusout(event) {
        let self = LGlobal.listViewDragObject;
        if (self) {
            self._ll_stop();
        }
    }
    _ll_stop() {
        ...
        self.stopDrag();
        self.isDeleted = true;
        if (Math.abs(LGlobal.offsetX - self.sx) < 5 && Math.abs(LGlobal.offsetY - self.sy) < 5) {
            listView.clickOnChild(LGlobal.offsetX - self.sx + self.selfX, LGlobal.offsetY - self.sy + self.selfY);
        }
